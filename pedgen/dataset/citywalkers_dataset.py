...
#================================================
    ...
    def load_scene_points_raw(self, label, intrinsics_old):
        ...
        try:
            ...
            #semantic_raw = np.array(semantic_raw, dtype=np.float32) / 18.0 #最大类别索引18，映射到[0.0, 1.0]
            semantic_raw = np.array(semantic_raw, dtype=np.float32)
            semantic_raw = tt(semantic_raw).unsqueeze(-1)#增加一维 h,w,1

            depth_3d = torch.cat([depth_3d, semantic_raw], dim=-1)#(X, Y, Z, 归一化语义值)

            voxel_dir = label["voxel"].rsplit("/", 1)[0]#label["voxel"]保存的是点云(X, Y, Z, 归一化语义值)
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        if self.mode == "train":
            img = torch.zeros([3, self.img_h, self.img_w], dtype=torch.float32)
        else:
            rgb = cv2.imread(label["image"])
            if rgb is None:
                raise FileNotFoundError(f"Failed to read image: {label['image']}")
            rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
            rgb = cv2.resize(rgb, (self.img_w, self.img_h), interpolation=cv2.INTER_LINEAR)
            rgb = rgb.astype(np.float32)
            mean = self.img_mean.reshape(1, 1, 3)
            std = self.img_std.reshape(1, 1, 3)
            rgb = (rgb - mean) / std
            img = torch.from_numpy(rgb).float().permute(2, 0, 1)
        ...
        data_dict["gt_init_pos"] = data_dict["global_trans"][0].clone()
        data_dict["gt_goal_rel"] = (
            data_dict["global_trans"][valid_len - 1] - data_dict["global_trans"][0]
        ).clone()

        # Global planner supervision with 30-frame stride and 60-frame horizon:
        ...
        last_valid = max(valid_horizon - 1, 0)#有效的帧数
        seg_pairs = [(0, 60), (30, 90), (60, 120), (90, 150)]
        init_anchor_idx = [0, 30, 60, 90]
        goal_seq = []
        valid_seq_mask = []
        init_seq = []
        init_seq_mask = []
        ...
        for start_idx, end_idx in seg_pairs:
            ...
            valid_seq_mask.append(1.0 if last_valid >= end_idx else 0.0)#根据序列有效长度last_valid生成Mask，标识哪些分段的GT是有监督意义的
        ...
        data_dict["scene_points_raw"] = self.load_scene_points_raw(label, intrinsics_old)
...
def collate_fn_pedmotion(data):
    ...
    for data_dict in data:
        ...
        if "scene_points_raw" in data_dict:
            scene_points_raw_batch.append(data_dict["scene_points_raw"])
    ...
    ret_dict = {
        ...
        "gt_init_pos": torch.stack(gt_init_pos_batch),
        "gt_goal_rel": torch.stack(gt_goal_rel_batch),
        "gt_init_pos_seq": torch.stack(gt_init_pos_seq_batch),
        "gt_init_pos_seq_mask": torch.stack(gt_init_pos_seq_mask_batch),
        "gt_goal_rel_seq": torch.stack(gt_goal_rel_seq_batch),
        "gt_goal_rel_seq_mask": torch.stack(gt_goal_rel_seq_mask_batch),
        "scene_points_raw": torch.stack(scene_points_raw_batch),
...
def collate_fn_pedmotion_pred(data):
    ...
    return collate_fn_pedmotion(data)
