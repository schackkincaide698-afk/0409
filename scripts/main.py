...
def build_scene_points(depth: np.ndarray, segmentation: np.ndarray,
                       intrinsics: np.ndarray, max_points: int) -> torch.Tensor:
    ...
    scene_points = torch.cat([depth_3d, semantic_raw], dim=-1).reshape(-1, 4)
    valid_mask = torch.isfinite(scene_points).all(dim=-1) & (scene_points[:, 2] > 1e-5)
    scene_points = scene_points[valid_mask]
    
    num_target = max_points
    num_points = scene_points.shape[0]

    if num_points == 0:
        scene_points = torch.zeros((num_target, 4), dtype=torch.float32)
    elif num_points >= num_target:
        ...
        scene_points = torch.cat([scene_points, scene_points[pad_indices]], dim=0)
...
def vis_smpl_impl(render, img, pred_vertices, output_png, output_mp4):
    img_vis = img.copy()
    colors = get_colors()
    num_frames = pred_vertices.shape[0]
    vis_indices = np.linspace(0, num_frames - 1, num=7, dtype=int)
    
    img_smpl, valid_mask = render.visualize_all(
        pred_vertices[vis_indices].cpu().numpy(),
        colors[[7, 6, 5, 4, 3, 2, 1]], # 确保颜色数组长度与采样的 7 个人影匹配
    )
...
def main():
    ...
    scene_points = build_scene_points(depth, segmentation, intrinsics, MAX_SCENE_POINTS)
    ...
    batch = {
        ...
        "scene_points_raw": torch.stack([scene_points.clone() for _ in range(num_pred_steps)]),
        "grid_size": torch.tensor(grid_size).float(),
        "grid_points": torch.tensor(grid_points).long(),
    }
    ...
    # --- 新增的多段航点打印逻辑 ---
    if "pre_init_pos_seq" in pred and "pre_goal_rel_seq" in pred:
        # 提取 Batch 中第一个样本的序列，维度为 [4, 3]
        init_seq = pred["pre_init_pos_seq"][0] 
        goal_seq = pred["pre_goal_rel_seq"][0] 
        num_segs = init_seq.shape[0]
        
        print(f"\n========== 预测全局 {num_segs} 段轨迹详情 (共 150 帧) ==========")
        for i in range(num_segs):
            start_sec = i * 1.0  # 因为每段重叠推进 30 帧(1秒)
            end_sec = start_sec + 2.0
            print(f"第 {i+1} 段 (时间窗: {start_sec:.1f}s - {end_sec:.1f}s):")
            print(f"  -> 绝对起点 (X, Y, Z): {init_seq[i].numpy().round(3)}")
            print(f"  -> 相对位移 (X, Y, Z): {goal_seq[i].numpy().round(3)}")
            print(f"  -> 绝对终点 (X, Y, Z): {(init_seq[i] + goal_seq[i]).numpy().round(3)}")
        print("==============================================================\n")
